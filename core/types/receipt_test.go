...
var (
	...
	ArsiaTestConfig = func() *params.ChainConfig {
...
func getArsiaTxReceipts(l1AttributesPayload []byte, l1GasPrice, l1BlobGasPrice, l1GasUsed, l1Fee *big.Int, baseFeeScalar, blobBaseFeeScalar, operatorFeeScalar, operatorFeeConstant *uint64, tokenRatio *big.Int) ([]*Transaction, []*Receipt) {
	// Create a few transactions to have receipts for
	txs := Transactions{
		NewTx(&DepositTx{
			To:    nil, // contract creation
...
func getBeforeArsiaTxReceipts(l1AttributesPayload []byte, l1GasPrice, l1GasUsed *big.Int, l1Fee *uint256.Int, feeScalar *big.Float, tokenRatio *big.Int) ([]*Transaction, []*Receipt) {
	// Create a few transactions to have receipts for
	txs := Transactions{
		NewTx(&DepositTx{
			To:    nil, // contract creation
...
func TestDeriveOptimismBedrockTxReceipts(t *testing.T) {
	...
	l1Fee := bedrockFee
	txs, receipts := getBeforeArsiaTxReceipts(payload, l1GasPrice, l1GasUsed, l1Fee, feeScalar, tokenRatio)
	...
	// Should get same result with the Ecotone config because it will assume this is "first Arsia block"
	// if it sees the bedrock style L1 attributes.
	err = Receipts(derivedReceipts).DeriveFields(ArsiaTestConfig, blockHash, blockNumber.Uint64(), 0, baseFee, nil, txs)
	if err != nil {
		t.Fatalf("DeriveFields(...) = %v, want <nil>", err)
	}
	checkBedrockReceipts(t, receipts, derivedReceipts)
...
func TestDeriveOptimismArsiaTxReceipts(t *testing.T) {
	...
	txs, receipts := getArsiaTxReceipts(payload, baseFee, blobBaseFee, minimumArsiaGas, ArsiaFee, &baseFeeScalarUint64, &blobBaseFeeScalarUint64, &operatorFeeScalarUint64, &operatorFeeConstantUint64, tokenRatio)
...
func TestDeriveOptimismArsiaTxReceiptsNoOperatorFee(t *testing.T) {
	// Arsia style l1 attributes with baseFeeScalar=2, blobBaseFeeScalar=3, baseFee=1000*1e6, blobBaseFee=10*1e6, operatorFeeScalar=7, operatorFeeConstant=9
	...
	baseFeeScalarUint64 := baseFeeScalar.Uint64()
	blobBaseFeeScalarUint64 := blobBaseFeeScalar.Uint64()
	txs, receipts := getArsiaTxReceipts(payload, baseFee, blobBaseFee, minimumArsiaGas, ArsiaFee, &baseFeeScalarUint64, &blobBaseFeeScalarUint64, nil, nil, tokenRatio)
...
func checkBedrockReceipts(t *testing.T, receipts, derivedReceipts []*Receipt) {
	...
	// Check that we preserved the invariant: l1Fee = l1GasPrice * l1GasUsed * l1FeeScalar * tokenRatio
	...
	var expectedL1Fee *big.Float
	if l2Rcpt.TokenRatio != nil {
		l1MNTFee := new(big.Float).Mul(l1Fee, new(big.Float).SetInt(l2Rcpt.TokenRatio))
		expectedL1Fee = l1MNTFee
	} else {
		expectedL1Fee = l1Fee
	}
	require.Equal(t, new(big.Float).SetInt(l2Rcpt.L1Fee), expectedL1Fee)
}
