...
func CheckMaxCodeSize(rules *params.Rules, size uint64, noMaxCodeSize bool) error {
	if rules.IsAmsterdam {
		if size > params.MaxCodeSizeAmsterdam && !noMaxCodeSize {
			return fmt.Errorf("%w: code size %v limit %v", ErrMaxCodeSizeExceeded, size, params.MaxCodeSizeAmsterdam)
		}
	} else if rules.IsEIP158 {
		if size > params.MaxCodeSize && !noMaxCodeSize {
