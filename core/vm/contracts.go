...
var PrecompiledContractsMantleEverest = map[common.Address]PrecompiledContract{
	...
	common.BytesToAddress([]byte{0x01, 0x00}): &p256VerifyEverest{},
}

var PrecompiledContractsMantleSkadi = map[common.Address]PrecompiledContract{
	common.BytesToAddress([]byte{0x01}):       &ecrecover{},
	common.BytesToAddress([]byte{0x02}):       &sha256hash{},
	common.BytesToAddress([]byte{0x03}):       &ripemd160hash{},
	common.BytesToAddress([]byte{0x04}):       &dataCopy{},
...
var PrecompiledContractsMantleLimb = map[common.Address]PrecompiledContract{
	common.BytesToAddress([]byte{0x01}): &ecrecover{},
	common.BytesToAddress([]byte{0x02}): &sha256hash{},
	common.BytesToAddress([]byte{0x03}): &ripemd160hash{},
	common.BytesToAddress([]byte{0x04}): &dataCopy{},
...
var PrecompiledContractsMantleArsia = map[common.Address]PrecompiledContract{
	common.BytesToAddress([]byte{0x01}): &ecrecover{},
	common.BytesToAddress([]byte{0x02}): &sha256hash{},
	common.BytesToAddress([]byte{0x03}): &ripemd160hash{},
	common.BytesToAddress([]byte{0x04}): &dataCopy{},
...
func init() {
	...
	for k := range PrecompiledContractsMantleEverest {
		PrecompiledAddressesMantleEverest = append(PrecompiledAddressesMantleEverest, k)
	}
	for k := range PrecompiledContractsMantleSkadi {
		PrecompiledAddressesMantleSkadi = append(PrecompiledAddressesMantleSkadi, k)
	}
...
func activePrecompiledContracts(rules params.Rules) PrecompiledContracts {
	...
	case rules.IsMantleSkadi:
		return PrecompiledContractsMantleSkadi
	case rules.IsMantleEverest:
		return PrecompiledContractsMantleEverest
	case rules.IsVerkle:
...
func ActivePrecompiles(rules params.Rules) []common.Address {
	...
	case rules.IsMantleSkadi:
		return PrecompiledAddressesMantleSkadi
	case rules.IsMantleEverest:
		return PrecompiledAddressesMantleEverest
	case rules.IsOsaka:
...
//
// This is used in the Mantle from Everest until the implementation of Ethereum's Osaka fork, after
// which the p256Verify precompile is used instead to maintain Ethereum equivalence.
type p256VerifyEverest struct {
	p256Verify
}

// RequiredGas returns the gas required to execute the precompiled contract
func (c *p256VerifyEverest) RequiredGas(input []byte) uint64 {
	return params.P256VerifyGasEverest
}
