...
func NewEVM(blockCtx BlockContext, statedb StateDB, chainConfig *params.ChainConfig, config Config) *EVM {
	...
	case evm.chainRules.IsMantleBaseFee: // for mantle base fee upgrade, activate shanghai opcode
...
func (evm *EVM) initNewContract(contract *Contract, address common.Address) ([]byte, error) {
	...
	if err := CheckMaxCodeSize(&evm.chainRules, uint64(len(ret)), evm.Config.NoMaxCodeSize); err != nil {
		return ret, err
	}
