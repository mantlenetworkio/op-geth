...
func (q *queue) DeliverBodies(id string, hashes eth.BlockBodyHashes, bodies []eth.BlockBody) (int, error) {
	...
	validate := func(index int, header *types.Header) error {
		...
		if header.WithdrawalsHash == nil {
			...
			if q.opConfig != nil && q.opConfig.IsOptimismWithSkadi(header.Time) {
				// If Skadi, we expect an empty list of withdrawal operations,
				// but the WithdrawalsHash in the header is used for the withdrawals state storage-root.
				if hashes.WithdrawalRoots[index] != types.EmptyWithdrawalsHash {
					return errInvalidBody
				}
			} else if hashes.WithdrawalRoots[index] != *header.WithdrawalsHash {
				return errInvalidBody
			}
		}
