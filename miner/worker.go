...
func (miner *Miner) commitTransactions(ctx context.Context, env *environment, plainTxs, blobTxs *transactionsByPriceAndNonce, interrupt *atomic.Int32) error {
	...
	for {
		...
		var err error
		if isMantleArsia {
			daFootprintLeft, err = checkMinDAFootprint(env)
			if err != nil {
		...
		if isMantleArsia {
			txDAFootprint, err = checkTxDAFootprint(env, daFootprintLeft, ltx.DABytes.Uint64(), ltx.Hash)
			if err != nil {
...
func (miner *Miner) fillTransactions(ctx context.Context, interrupt *atomic.Int32, env *environment) (err error) {
	...
	if !miner.chainConfig.IsOptimism() && miner.chainConfig.IsOsaka(env.header.Number, env.header.Time) && !miner.chainConfig.IsAmsterdam(env.header.Number, env.header.Time) {
	...
	preconfTxs, pendingPlainTxs, plainTxCount := miner.txpool.PendingWithPreconfTxs(filter)
	log.Debug("find preconf txs to fill into block", "count", len(preconfTxs))
	var unsealedPreconfTxs []*types.Transaction
	if len(preconfTxs) > 0 {
		unsealedTxs, err := miner.commitFIFOTransactions(ctx, env, preconfTxs, interrupt)
		if err != nil {
			return err
		}
	...
	span.SetAttributes(
		telemetry.Int64Attribute("pending.preconf.count", int64(len(preconfTxs))),
...
func (miner *Miner) commitFIFOTransactions(ctx context.Context, env *environment, txs []*types.Transaction, interrupt *atomic.Int32) ([]*types.Transaction, error) {
	ctx, _, spanEnd := telemetry.StartSpan(ctx, "miner.commitFIFOTransactions")
	defer spanEnd(nil)
...
// OP-Stack addition: check DA footprint limit
func checkDAFootprint(env *environment, txDASize uint64, txHash common.Hash) (uint64, error) {
	...
	txDAFootprint, err := checkTxDAFootprint(env, daFootprintLeft, txDASize, txHash)
	if err != nil {
		return 0, err
	}

	return txDAFootprint, nil
}

func checkMinDAFootprint(env *environment) (uint64, error) {
	minTransactionDAFootprint := types.MinTransactionSize.Uint64() * uint64(env.daFootprintGasScalar)

	// Calculate DA footprint remaining space
	daFootprintLeft := env.header.GasLimit - *env.header.BlobGasUsed
	if daFootprintLeft < minTransactionDAFootprint {
		return 0, fmt.Errorf("not enough DA space for further transactions (have=%d want=%d): %w",
			daFootprintLeft, minTransactionDAFootprint, core.ErrDAFootprintLimitReached)
	}

	return daFootprintLeft, nil
}

func checkTxDAFootprint(env *environment, daFootprintLeft, txDASize uint64, txHash common.Hash) (uint64, error) {
	...
	txDAFootprint := txDASize * uint64(env.daFootprintGasScalar)
	if daFootprintLeft < txDAFootprint {
		return 0, fmt.Errorf("not enough DA space left for transaction %s (left=%d needed=%d): %w",
			txHash.Hex(), daFootprintLeft, txDAFootprint, core.ErrDAFootprintLimitReached)
	}
