...
var (
	...
	// Pre-confirm transaction journal
	PreconfTxJournalGauge = metrics.GetOrRegisterGauge("preconf/txpool/journal", nil)
)
...
func LogIfSlow(start time.Time, msg string, ctx ...any) {
	...
	if duration > SlowThreshold {
		log.Warn("Slow "+msg, append([]any{"duration.ms", duration.Milliseconds(), "elapsed", duration}, ctx...)...)
	}
}
